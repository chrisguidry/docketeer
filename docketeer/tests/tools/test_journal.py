"""Tests for journal tools (add and entries)."""
...
async def test_journal_entries_today(tool_context: ToolContext):
    today = datetime.now().astimezone().strftime("%Y-%m-%d")
    journal = tool_context.workspace / "journal"
    journal.mkdir()
    (journal / f"{today}.md").write_text(f"# {today}\n\n- 10:00 | today entry\n")

    result = await registry.execute("journal_entries", {}, tool_context)
    assert "today entry" in result


async def test_journal_entries_specific_date(tool_context: ToolContext):
    journal = tool_context.workspace / "journal"
    journal.mkdir()
    (journal / "2026-01-15.md").write_text("# 2026-01-15\n\n- 10:00 | old entry\n")

    result = await registry.execute(
        "journal_entries", {"date": "2026-01-15"}, tool_context
    )
    assert "old entry" in result


async def test_journal_entries_date_not_found(tool_context: ToolContext):
    journal = tool_context.workspace / "journal"
    journal.mkdir()
    result = await registry.execute(
        "journal_entries", {"date": "2020-01-01"}, tool_context
    )
    assert "No journal for 2020-01-01" in result


async def test_journal_entries_range(tool_context: ToolContext):
    journal = tool_context.workspace / "journal"
    journal.mkdir()
    ...
    result = await registry.execute(
        "journal_entries", {"start": "2026-01-10", "end": "2026-01-15"}, tool_context
    )
...
async def test_journal_entries_range_start_filter(tool_context: ToolContext):
    journal = tool_context.workspace / "journal"
    journal.mkdir()
    ...
    result = await registry.execute(
        "journal_entries", {"start": "2026-01-10"}, tool_context
    )
    assert "in range" in result
    assert "too early" not in result


async def test_journal_entries_range_empty(tool_context: ToolContext):
    journal = tool_context.workspace / "journal"
    journal.mkdir()
    result = await registry.execute(
        "journal_entries", {"start": "2099-01-01", "end": "2099-12-31"}, tool_context
    )
    assert "No journal entries for range" in result


async def test_journal_entries_no_journal_dir(tool_context: ToolContext):
    result = await registry.execute("journal_entries", {}, tool_context)
