"""Tests for migration 001: backstage JSON to workspace markdown."""
...
# Load the migration module (can't import directly — filename starts with a digit)
_m001_path = next(p for n, p in _discover() if n == 1)
_m001 = _load_module(_m001_path)
_convert_tuning = _m001._convert_tuning
_convert_mcp = _m001._convert_mcp
run = _m001.run
...
def test_run_tuning(tmp_path: Path):
...
def test_run_mcp(tmp_path: Path):
    data_dir = tmp_path / "data"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
...
def test_run_both_dirs(tmp_path: Path):
    data_dir = tmp_path / "data"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
