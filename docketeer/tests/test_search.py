"""Tests for the search index protocol, NullSearch, and discovery."""

from unittest.mock import MagicMock, patch

from docketeer.search import NullSearch, discover_search
...
def test_discover_search_returns_null_when_no_plugin():
    with patch("docketeer.search.discover_one", return_value=None):
        search = discover_search()
    assert isinstance(search, NullSearch)


def test_discover_search_returns_plugin_search():
    fake_search = NullSearch()
    module = MagicMock()
    module.create_search.return_value = fake_search
